...
public interface SongRepository extends CrudRepository<SongModel, Integer> {
    ...
    @Query(value = "SELECT song.id, title, year, duration, filePath, band.id as bandId, band.name as bandName,manager.id as managerId,manager.name as managerName,supplier.id as supplierId,supplier.name as supplierName FROM song INNER JOIN band ON song.bandId = band.id INNER JOIN manager ON song.managerId = manager.id INNER JOIN supplier ON song.supplierId = supplier.id WHERE (:title is null or song.title LIKE %:title%) AND (:band is null or band.name LIKE %:band%) AND (:manager is null or manager.name LIKE %:manager%) AND (:supplier is null or supplier.name LIKE %:supplier%)",nativeQuery = true)
